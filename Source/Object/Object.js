...
/*
  ...
  property. Callback observers recieve new and old value each time value
  changes.
*/
...
*/
    ...
    if (stored && (stored = stored[key])) for (var i = 0, args; args = stored[i++];) {
      obj = args[0], mem = args[2], same = obj === value;
      if (value != null && !same && (!mem || !mem._delegate || !mem._delegate(value, key, obj, true)))
      ...
      if (old != null && obj !== old && !same && (!mem || !mem._delegate || !mem._delegate._delegate(old, key, obj, false)))
...
/*
  Unset method clean object key resetting its value to undefined. It does all
  things that .set do in the same order: hashes its key, transforms values,
  notifies observers and fires callbacks.
*/
...
/*
  ...
  `this.constructor` which creates the same kind of object.
  When an object is mixed with merge mode turned on, and there was no
  object by the same key already, mix will attempt to reference a merged
  object marking it for a copy-on-write. It means that merged object
  will be the same object as was given in arguments, but there will
  be a copy made on any attempt to modify the object (e.g. by merging
  another object by the same key).
*/
    var obj = this[key];
    if (obj == null) {
...
/*
  ...
  write a reference to the object instead of creating a full copy
  of an object. Although the copy will be created if an outside object
...
/*
  ...
  unmixing objects can be explored with LSD.Stack, that allows objects to be
  ...
  to unset the ones that match the given values. LSD.Stack on the other hand,
  is pretty strict and never loses a value that was set before, and can
  ...
  it overrides fourth given argument with `false` when calling `mix`. It
  comes in handy when using stored arguments that may be processed with
...
/*
  ...
  same as simple mix, but it also tries to subscribe current object to
...
/*
  ...
  right time. LSD does not care about type of callback while observing if it
  isn't the right time to call it. All endpoints that might call outsider's
  callbacks in LSD.Object execute `_callback` method that figures out how to
  dispatch the given callback. It adds to consistensy and API richness across
  all observable structures. Another detail that makes it more practical is
  the fact that each time a callback is called with a new value, it also
  sends the old value that was possibly overriten. There may not be a
...
/*
  ...
  opt out of writing or claiming `_owner` reference, but it is there
...
LSD.Object.prototype._construct = function(name, constructor, memo, value) {
  ...
  if (!this.onBeforeConstruct || typeof (instance = this.onBeforeConstruct(name, constructor)) == 'undefined') {
    ...
    this.set(name, instance, this._delegate && !memo ? this : memo);
  }
  return instance;
};
