...
LSD.Layout.prototype = Object.append(new Options, {
  ...
  children: function(children, parent, opts, memo) {
    if (!memo) memo = {};
    var stack = memo.stack = this.getMutationStack(parent[1] || parent.element, parent, memo.stack);
  ...
  selector: function(string, parent, opts) {
    ...
    if (options.tag != '*' && (options.source || this.context.find(options.tag) || !LSD.Layout.NodeNames[options.tag])) {
      ...
      opts = options;
      if (nodes && nodes.length) this.children(nodes, [widget.element, widget], opts);
    } else {  
  ...
  keyword: function(text, parent, opts, memo, element) {
    ...
    switch (parsed.keyword) {
      case "if":
        var options = {expression: parsed.expression};
        break;
      case "unless":  
        var options = {expression: parsed.expression, invert: true};
        break;
      case "elsif": case "elseif": case "else if":
        var options = {expression: parsed.expression, link: true};
        break;
      case "else":
      ...
      case "build":
        ...
        break;
      case "end":
        if (!element) throw "Unexpected END keyword. END must be used in DOM and can not be used in object layouts."
        var options = {ends: true}
    }
  ...
  */
  ...
  getMutationStack: function(element, parent, stack) {
    var widget = stack ? (parent[1] && (parent[1] == parent[0].element)) ? parent[0] : null : parent[0] || parent;
    var group;
    if (stack) {
      /* 
  ...
  walk: function(element, parent, opts, memo) {
    ...
    */
    ...
    if (!stack) stack = memo.stack = this.getMutationStack(element, parent, memo.stack);
    /* 
...
LSD.Layout.Branch.prototype = Object.append({
  ...
  set: function(value) {
    this[(value ^ this.options.invert) ? 'check' : 'uncheck']();
  },
  show: function() {
...
LSD.Layout.rExpressions = /^\s*(if|else|call|els[e ]*if|unless|build|template|end)(?:$|\s+(.*?)\s*$)/
