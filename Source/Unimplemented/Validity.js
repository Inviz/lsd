...
LSD.Mixin.Validity.events = {
  invalidate: function(message) {
    var options = LSD.Mixin.Validity.message;
    if (this.attributes.id) options = Object.merge({attributes: {'for': this.attributes.id}}, LSD.Mixin.Validity.message);
    this.allocate('message', 'error', options, message)
  },
